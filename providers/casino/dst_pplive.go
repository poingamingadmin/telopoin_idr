...
type DstPPLive struct {
	ApiURL      string
	CompanyCode string
	ApiKey      string
}

func (p *DstPPLive) StartGame(req providers.LaunchRequest) (string, error) {
	start := time.Now()
...
func init() {
	...
	if apiURL == "" || companyCode == "" || apiKey == "" {
		panic("❌ ENV DstPPLive_API_URL, DstPPLive_COMPANY_CODE, or DstPPLive_API_KEY not set")
	}

	providers.RegisterProvider("PPLCO", &DstPPLive{
		ApiURL:      apiURL,
