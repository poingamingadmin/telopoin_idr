...
type Win568AsiaGaming struct {
	ApiURL     string
	CompanyKey string
	ServerID   string
}

func (p *Win568AsiaGaming) StartGame(req providers.LaunchRequest) (string, error) {
	start := time.Now()
...
func init() {
	...
	providers.RegisterProvider("PLAYACE", &Win568AsiaGaming{
		ApiURL:     apiURL,
