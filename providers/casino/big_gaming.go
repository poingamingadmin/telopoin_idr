...
type Win568BigGaming struct {
	ApiURL     string
	CompanyKey string
	ServerID   string
}

func (p *Win568BigGaming) StartGame(req providers.LaunchRequest) (string, error) {
	start := time.Now()
...
func init() {
	...
	providers.RegisterProvider("BigGaming", &Win568BigGaming{
		ApiURL:     apiURL,
