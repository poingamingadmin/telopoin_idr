...
type Win568SexyGaming struct {
	ApiURL     string
	CompanyKey string
	ServerID   string
}

func (p *Win568SexyGaming) StartGame(req providers.LaunchRequest) (string, error) {
	start := time.Now()
...
func init() {
	...
	providers.RegisterProvider("AeSexy", &Win568SexyGaming{
		ApiURL:     apiURL,
