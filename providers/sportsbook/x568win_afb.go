...
type Win568AFB struct {
	ApiURL     string
	CompanyKey string
	ServerID   string
}

func (p *Win568AFB) StartGame(req providers.LaunchRequest) (string, error) {
	start := time.Now()
	...
	finalURL := fmt.Sprintf("https://%s&lang=%s&gpId=1015&device=%s",
...
func init() {
	...
	providers.RegisterProvider("afb", &Win568AFB{
		ApiURL:     apiURL,
