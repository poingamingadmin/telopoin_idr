...
type Win568Saba struct {
	ApiURL     string
	CompanyKey string
	ServerID   string
}

func (p *Win568Saba) StartGame(req providers.LaunchRequest) (string, error) {
	start := time.Now()
	...
	finalURL := fmt.Sprintf("https://%s&lang=%s&gpId=44&device=%s",
...
func init() {
	...
	providers.RegisterProvider("saba", &Win568Saba{
		ApiURL:     apiURL,
