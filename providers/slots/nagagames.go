...
type Win568NagaGames struct {
	ApiURL     string
	CompanyKey string
	ServerID   string
}

func (p *Win568NagaGames) StartGame(req providers.LaunchRequest) (string, error) {
	start := time.Now()
...
func init() {
	...
	providers.RegisterProvider("NagaGames", &Win568NagaGames{
		ApiURL:     apiURL,
