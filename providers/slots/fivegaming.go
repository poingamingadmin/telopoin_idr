...
type Win568FiveGaming struct {
	ApiURL     string
	CompanyKey string
	ServerID   string
}

func (p *Win568FiveGaming) StartGame(req providers.LaunchRequest) (string, error) {
	start := time.Now()
...
func init() {
	...
	providers.RegisterProvider("FiveGaming", &Win568FiveGaming{
		ApiURL:     apiURL,
